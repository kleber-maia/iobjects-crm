...
/**
 * Representa o relatório de RelatorioRankingOportunidadeCampanha.
 */
public class RelatorioRankingOportunidadeCampanha extends Report {

  // identificação da classe
  static public final String CLASS_NAME = "imanager.report.RelatorioRankingOportunidadeCampanha";
  // nossa ajuda extra
  static public final String HELP = "";
  // nossas ações
  static public final Action ACTION               = new Action("relatorioRankingopOrtunidadeCampanha", "Oportunidade por Campanha", "Emite o relatório de Ranking Oportunidade por Campanha.", HELP, "report/relatoriorankingoportunidadecampanha.jsp", "CRM", "Oportunidade", Action.CATEGORY_REPORT, false, true);
  static public final Action ACTION_RELATORIO     = new Action("relatorioRankingopOrtunidadeCampanhaRelatorio", ACTION.getCaption(), ACTION.getDescription(), HELP, "report/relatoriorankingoportunidadecampanharelatorio.jsp", ACTION.getModule(), ACTION.getAccessPath(), Action.CATEGORY_REPORT, ACTION.getMobile(), false);
  // nossos comandos
   ...
   * Construtor padrão.
   */
  public RelatorioRankingOportunidadeCampanha() {
    // nossas ações
  ...
  /**
   * Retorna o ResultSet de RelatorioRankingOportunidadeCampanha.
   ...
   * @return ResultSet Retorna o ResultSet de RelatorioRankingOportunidadeCampanha.
   ...
   *                   dados.
   */
  public ResultSet getResultSetRelatorioRankingOportunidadeCampanha(
                       int empresaId,
                       int usuarioId,
                       int departamentoId,
                       int faseId,
    ...
    try {
      // SQL
      String sql = "SELECT crm_campanha.va_nome as nome, count(crm_oportunidade.in_campanha_id) as total_oportunidade "
                 ...
                 + " AND ((crm_oportunidade.in_usuario_id = ?) OR (" + usuarioId + " = 0 )) "
                 ...
                 + "ORDER BY total_oportunidade DESC, nome ";
      // prepara a query
  ...
  public ResultSet getResultSetCampanhas(
    ...
    try {
      ...
      String sql = "SELECT crm_campanha.va_nome as nome_campanha, crm_oportunidade.in_oportunidade_id, global_departamento.va_nome as nome_departamento, securityservice_usuario.va_nome as nome_usuario, relacionamento_contato.va_nome as nome_cliente, crm_fase.va_nome as nome_fase, crm_oportunidade.da_acompanhamento, crm_oportunidade.sm_percentual_sucesso as sucesso, crm_oportunidade.sm_status as status, crm_oportunidade.do_valor as valor, crm_oportunidade.in_empresa_id as empresa_id "
                 ...
                 + " AND (crm_oportunidade.da_inclusao >= ? "
                 + " AND crm_oportunidade.da_inclusao <= ?) "
                 + "ORDER BY nome_campanha, crm_oportunidade.in_oportunidade_id ";
      // prepara a query
      PreparedStatement preparedStatement = prepare(sql);
      // preenche os parâmetros
      preparedStatement.setInt(1, empresaId);
